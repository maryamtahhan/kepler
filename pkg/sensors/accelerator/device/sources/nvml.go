...
func (n *GPUNvml) GetProcessResourceUtilizationPerDevice(device dev.Device, since time.Duration) (map[uint32]dev.ProcessUtilizationSample, error) {
	...
	if n.processUtilizationSupported {
		...
		if ret != nvml.SUCCESS {
			...
			for _, pinfo := range processUtilizationSample {
				// pid 0 means no data.
				if pinfo.Pid != 0 {
					processAcceleratorMetrics[pinfo.Pid] = dev.ProcessUtilizationSample{
						Pid:         pinfo.Pid,
	...
	if !n.processUtilizationSupported { // if processUtilizationSupported is false, try deviceGetMPSComputeRunningProcesses_v3 to use memory usage to ratio power usage
		...
		if ret != nvml.SUCCESS {
			...
			return nil, fmt.Errorf("failed to get processes' utilization on device %v: %v", device.ID, nvml.ErrorString(ret))
		}
		memoryInfo, ret := device.DeviceHandler.(nvml.Device).GetMemoryInfo()
		if ret != nvml.SUCCESS {
